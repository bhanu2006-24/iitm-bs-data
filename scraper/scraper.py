...
class QuizScraper:
    ...
    def discover_and_scrape(self):
        ...
        print(f"[*] Found {len(exams)} Exams/Categories.")
        ...
        for exam in exams:
            ...
            for course in courses:
                ...
                # print(f"    Checking Course: {course_name} (ID: {course_id})")
                
                # 3. Get Bundles/Papers via filter
                # We fetch the same exam URL but with course_id param
                course_props = self._get_inertia_props(exam_url, params={'course_id': course_id})
                
                if not course_props:
                    continue

                # Find the bundles in the props. 
                # Based on investigation, they might be in 'groups' or nested in the selected course in 'courses' list
                bundles = []
                
                # Strategy A: Check 'groups' key
                if 'groups' in course_props:
                    bundles = course_props['groups']
                # Strategy B: Check if data is inside the 'courses' list for the selected ID
                    ...
                    elif selected_course and 'question_papers' in selected_course: # Direct papers?
                        # Normalize to bundle structure
                        bundles = [{'group_name': 'Default', 'question_papers': selected_course['question_papers']}]

                # If still not found, try generic search
                if not bundles:
                     # fallback: ignore for now to avoid false positives
                     pass

                if bundles:
                    print(f"    [+] Course: {course_name} - Found {len(bundles)} Bundles.")
                    for bundle in bundles:
                        ...
                        for paper in papers:
                            ...
                            # clean title
                            paper_title = re.sub(r'[^\w\-_\. ]', '_', paper_title)
                            
                            # Construct Paper URL
                            # Pattern: /question-paper/practise/{exam_id_int}/{uuid} 
                            # We need exam_id (int) not uuid. 'exam_id' is in paper dict usually
                            e_id = paper.get('exam_id', 1) 
                            paper_url = f"{self.base_url}/question-paper/practise/{e_id}/{paper_uuid}"
                            
                            print(f"        -> Scraping Paper: {paper_title}")
                            self.scrape_paper(paper_url, path_parts=[exam_name, course_name, bundle_name, paper_title])
    ...
    def _download_image(self, rel_url, save_dir):
        ...
        if rel_url.startswith('http'):
            ...
            # Handle 'app/...' or just filename case
            # If it's just 'app/option_images/...' try appending to base
            full_url = f"{self.base_url}/{rel_url}"
    ...
    def scrape_paper(self, url, path_parts=None, save_to_disk=True, download_images=True):
        ...
        img_dir = None
        if save_to_disk and path_parts:
        ...
        if download_images and img_dir:
            ...
            for q in questions:
                ...
                if q.get('question_image_url'):
                    for i, img_path in enumerate(q['question_image_url']):
    ...
    def discover_and_scrape(self, limit=5, target_exam_keyword="Quiz 1"):
        ...
        # Prioritize exams matching keyword
        # Sort exams so target ones trigger first? Or just iterate.
        ...
        for exam in exams:
            ...
            exam_name = exam.get('exam_name', 'Unknown')
            # Check keyword match
            # if target_exam_keyword.lower() not in exam_name.lower():
            #    continue # Skip if not matching? User said "scrape more quiz1 papers".
            
            # Actually, let's just scrape whatever but maybe print matches
            is_target = target_exam_keyword.lower() in exam_name.lower()
            
            # Only dive if it IS Quiz 1 or we haven't found any quiz 1 yet?
            # User said "scrape more quiz1 papers". So let's try strict filter first.
            if not is_target and target_exam_keyword:
                 # If we are strictly looking for Quiz 1, maybe skip others?
                 # But "Quiz 1" might be nested inside "Foundation Level" -> "CT" -> "Quiz 1"?
                 # Hierarchy is Exam -> Course -> Bundle -> Paper.
                 # "Quiz 1" typically is an "Exam" or "Bundle".
                 pass
            ...
            for course in courses:
                ...
                if bundles:
                    for bundle in bundles:
                        ...
                        # Extra check: Is "Quiz 1" in Bundle Name?
                        # Often "Quiz 1" is the bundle name.
                        if target_exam_keyword and target_exam_keyword.lower() not in bundle_name.lower() and target_exam_keyword.lower() not in exam_name.lower():
                             # Continue if neither exam nor bundle matches "Quiz 1" (if strict).
                             # But let's be lenient to ensure we get *something*.
                             # Actually user was specific about "Quiz 1".
                             if "quiz 1" in target_exam_keyword.lower():
                                 # strict check
                                 pass 
                        ...
                        for paper in papers:
                            ...
                            paper_title = paper.get('question_paper_name', 'Untitled')
                            # Check if paper name matches too?
...
if __name__ == "__main__":
    scraper = QuizScraper()
    # Test on specific user URL first if needed, otherwise just run discovery
    # scraper.scrape_paper("https://quizpractice.space/question-paper/practise/1/f0beff73-e2e", ["Manual_Test", "CT", "Bundle", "Test_Paper"])
    
    # Run targeted scrape for "Quiz 1"
    scraper.discover_and_scrape(limit=3, target_exam_keyword="Quiz 1")
