...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if(headerRight) {
      ...
      editControls.className = 'flex items-center gap-2 mr-4 bg-slate-800 rounded-lg p-1 border border-slate-700';
  ...
  if (mode === "preview") {
    try {
      ...
      examTitle.textContent = "PREVIEW MODE: " + (currentExam.title || "Untitled");
      ...
      banner.className = "fixed top-0 left-0 right-0 z-50 bg-purple-600 text-white text-xs font-bold text-center py-1";
  ...
  if (toggleEditBtn) {
      ...
      saveChangesBtn.addEventListener('click', async () => {
          if (!confirm("Save changes to this paper? This cannot be undone.")) return;
          try {
  ...
  function updatePalette() {
    questions.forEach((q, idx) => {
      ...
      if (isCurrent) classes += "border-blue-500 ring-2 ring-blue-500/30 ring-offset-2 ring-offset-dark z-10 ";
      else classes += "border-transparent ";

      if (status === "answered") classes += "bg-green-600 text-white hover:bg-green-500";
      else if (status === "review") classes += "bg-purple-600 text-white hover:bg-purple-500";
      else if (status === "visited") classes += "bg-red-500/20 text-red-400 border-red-500/30 hover:bg-red-500/30";
      else classes += "bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700";
    ...
    if (isEditMode) {
         contentHtml = `
            <div class="mb-6 p-4 border border-blue-500/50 bg-blue-500/5 rounded-xl">
                ...
                <textarea id="edit-q-text" class="w-full bg-slate-900 border border-slate-700 text-slate-200 p-3 rounded font-mono text-sm h-32 outline-none focus:border-blue-500">${q.text}</textarea>
                
                <div class="mt-4 grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-blue-400 text-xs font-bold uppercase mb-2 block">Marks</label>
                        <input id="edit-q-marks" type="number" value="${q.marks}" class="bg-slate-900 border border-slate-700 text-white p-2 rounded w-full outline-none">
                    </div>
                    <div>
        ...
        if (['MCQ', 'MSQ'].includes(q.type)) {
            ...
            (q.options || []).forEach((opt, idx) => {
                 const isCorrect = params.get('mode') === 'preview' ? false : (Array.isArray(q.correctIndex) ? q.correctIndex.includes(idx) : q.correctIndex === idx);
                 contentHtml += `
                    <div class="flex gap-2 items-center">
                        <input type="checkbox" class="accent-green-500 w-5 h-5" id="edit-opt-correct-${idx}" ${isCorrect ? 'checked' : ''}>
                        <input type="text" class="flex-1 bg-slate-900 border border-slate-700 text-slate-300 p-2 rounded text-sm" value="${opt.replace(/"/g, '&quot;')}" id="edit-opt-text-${idx}">
            ...
            `;
             ...
             contentHtml += `
                <div>
                    ...
                    <input id="edit-nat-val" type="text" value="${q.correctValue || ''}" class="bg-slate-900 border border-slate-700 text-white p-2 rounded w-full outline-none">
                </div>
             `;
        }
        ...
        setTimeout(() => {
            ...
            if(typ) typ.onchange = (e) => { q.type = e.target.value; renderQuestion(q); };
  ...
  window.updateCorrectIndex = (q) => {
    ...
    if (q.type === 'MCQ') q.correctIndex = checked.length > 0 ? checked[0] : null;
  ...
  function updateNavButtons() {
    ...
    nextBtn.innerHTML = currentIndex === questions.length - 1 ? 'Finish Review' : 'Next <i class="fas fa-chevron-right ml-1"></i>';
